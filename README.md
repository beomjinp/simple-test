...
- api 서버 소스는 본 프로젝트를 fork한 후 '아이디_srv' 로 branch를 생성한다. ex) 아이디가 smoh일 경우 브랜치 이름은 smoh_srv
- 안드로이드 앱 소스는 본 프로젝트를 fork한 후 '아이디_app' 으로 branch를 생성. ex) 아이디가 smoh일 경우 smoh_app
- 생성한 브랜치에서 작업한 후 동일 브랜치로 pull request를 보낸다.
